...
export const Item: React.FC<ItemProp> = ({ itemNames, scroll, showMen }) => {
    const scrollToSection = (id: string) => {
        ...
        (id === 'Proyectos' || id === 'Experiencia') ? section?.scrollIntoView({ behavior: 'smooth', block: "start" }) 
    ...
    <>
        {itemNames.map(item => {
            ...
            return (
                ...
                >
                    {IconComponent && <IconComponent className='3xl:w-[25px] 2xl:w-[20px] xl:w-[17px] lg:w-[15px]'/>} 
                    <span className={`${(scroll && !showMen) ? `hidden group-hover:block group-hover:text-primary` 
                        : ` xl:text-button-xl 2xl:text-button-xl 3xl:text-button lg:block 3xl:block 2xl:block xl:block
